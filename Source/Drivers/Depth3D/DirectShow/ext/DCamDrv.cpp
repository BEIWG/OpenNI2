...
{
	//uint32_t ref_baseaddr = 0x400000;
	//ifstream refin("../UVCExU/ref.bin", ios::in | ios::binary);
	ifstream refin(reffilename, ios::in | ios::binary);
	//uint32_t ref_baseaddr = 0x000000;
	//ifstream refin("../../HDL/par/vvdprj_sl1000_35t/vvdprj_sl1000_35t.runs/impl_1/SL1000_V3.bin", ios::in | ios::binary);
	
	refin.seekg(0, refin.end);
	size_t srcSize = refin.tellg();
	refin.seekg(0);

	printf("srcSize = %d \r\n", srcSize);
	uint8_t * refdata = new uint8_t[srcSize];
	uint8_t tmp_data[256];
	refin.read((char *)refdata, srcSize);
